...
class GradeCalculatorCubit extends Cubit<GradeCalculatorState> {
  GradeCalculatorCubit() : super(const GradeCalculatorState());

  void initializeGrades(int gradeCount) {
    final suggestedPercentages = GradeCalculation.getSuggestedPercentages(gradeCount);
  ...
  void addGrade() {
    ...
    final suggestedPercentages = GradeCalculation.getSuggestedPercentages(newGradeCount);
    final grades = List.generate(
      newGradeCount,
      (index) => Grade(
  ...
  void removeGrade(int index) {
    if (state.grades.length <= 2) return; // Minimum 2 grades
    
    final grades = List<Grade>.from(state.grades);
    grades.removeAt(index);
    
    // Redistribute percentages
    final suggestedPercentages = GradeCalculation.getSuggestedPercentages(grades.length);
  ...
  void resetCalculator() {
    emit(const GradeCalculatorState());
  }
}